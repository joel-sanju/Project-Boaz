...
if($_SESSION['role'] == '9') $kzn =  1; else die(header("Location: ./home.php")); 
...
.delete_n{margin:3px; padding-right:20px; color:#F60; font-weight:bolder; cursor:pointer}
  ...
  <body>
   ...
   <!-- Item Section -->
     ...
     <h5 class="card-header">Manage Admins <a style="float:right;" href="add-user.php" class="btn btn-success">Add</a></h5>
								 ...
								 $pdo = new mypdo();               
								  ...
								  for($iz; $iz < count($rowz); $iz++){  $row = $rowz[$iz];?>
								                   <tr data-state="<?php echo $row['level']; ?>" id="row_<?php echo $row['id']; ?>">
                                                        <td><?php echo $row['fname']; ?></td>
                                                        <td><?php echo $row['email']; ?></td>
                                                        <td><?php if($row['level'] == 8) echo 'High'; elseif($row['level'] == 9) echo 'Super'; elseif($row['level'] == 1) echo 'Low'; elseif($row['level'] == 3) echo 'Deactivated User'; elseif($row['level'] == 0) echo 'New User (Activate this User)';?></td>

                                                        <td>
                                                          <button onClick="open_operation(<?php echo $row['id']; ?>)" style="float:right;" class="btn btn-info fa fa-gears"> Operation</button>
